...
export class InterfaceScanner {
	...
	getDomainsWithJoint(joint: string): string[] {
		...
		return fs
			...
			.filter((e) => fs.existsSync(path.join(this.libDir, e.name, 'interface', joint)))
			.map((e) => e.name)
			.sort();
	}

	scanFiles(ifaceDir: string): string[] {
		const result: string[] = [];
		if (!fs.existsSync(ifaceDir)) return result;
		this.#walk(ifaceDir, (full, src) => {
			if (/export interface \w+/.test(src)) result.push(full);
		});
		return result.sort();
	}
	...
	extractKeys(content: string, pos: number): string[] {
	...
	extractEntries(content: string, pos: number): Record<string, string> {
		let depth = 0, end = -1, inStr = false, strCh = '';
		for (let i = pos; i < content.length; i++) {
			const c = content[i];
			if (inStr) { if (c === '\\') { i++; continue; } if (c === strCh) inStr = false; continue; }
		...
		let m: RegExpExecArray | null;
		while ((m = re.exec(body)) !== null) entries[m[1]] = m[2].trim();
		return entries;
	}
